...
import {SecretKey, SigningTranscript} from '../../src/signingContext'
import {b} from '../merlin/utils'
import {Keypair} from '../../src/keypair'
import {Scalar, ScalarAdd, ScalarBigintToBytesForm, ScalarBytesToBigintForm} from '../../src/scalar'

const INVALID_MNEMONIC = 'Invalid mnemonicToMiniSecret'
const INVALID_ENTROPY = 'Invalid entropy'
const INVALID_CHECKSUM = 'Invalid mnemonicToMiniSecret checksum'
...
const WORDS_MAP = DEFAULT_WORDLIST.split('|').reduce((acc, elem, index) => {
  acc[elem] = index
  return acc
...
export const mnemonicToEntropy = (mnemonic: string): Uint8Array => {
  ...
  const bits = words
    .map((word): string => {
      if (!WORDS_MAP.hasOwnProperty(word)) {
        throw new Error(INVALID_MNEMONIC)
      }
  ...
  const entropyBytes = matched && matched.map(bin => parseInt(bin, 2))
...
export function entropyToMnemonic(entropy: Uint8Array): string {
  ...
  const mapped = matched && matched.map((binary) => DEFAULT_WORDLIST[parseInt(binary, 2)])
...
export const deriveHard = (secretKey: Uint8Array, chainCode: Uint8Array): Keypair => {
  ...
  transcript.append_message(b`secret-key`, secretKey)
