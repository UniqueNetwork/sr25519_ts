...
export class FieldElement
{
    ...
    {
        var low51BitMask: bigint = (1n << 51n) - 1n;
    ...
    {
        var nself = this.Negate();
        this.ConditionalAssign(nself, choice);
    }

    IsNegative(): boolean
    {
        var dt = this.data[0] % 256n;
        var dti = Number(dt) & 1;
        return dti > 0;
    }
    ...
    Pow2k(k: number): FieldElement
    {
        ...
        while (true)
        {
            ...
            var t1 = (m(a[1], a4_19) + m(a[2], a3_19));
            var t2 = 2n * t1 ;
            var t3 = m(a[0], a[0]) + t2;
            var c0 = m(a[0], a[0]) + 2n * (m(a[1], a4_19) + m(a[2], a3_19));
            var c1 = m(a[3], a3_19) + 2n * (m(a[0], a[1]) + m(a[2], a4_19));
            var c2 = m(a[1], a[1]) + 2n * (m(a[0], a[2]) + m(a[4], a3_19));
            var c3 = m(a[4], a4_19) + 2n * (m(a[0], a[3]) + m(a[1], a[2]));
            var c4 = m(a[2], a[2]) + 2n * (m(a[0], a[4]) + m(a[1], a[3]));
    ...
    Mul(second: FieldElement): FieldElement
    {
        ...
        //                       = 55.25 + b,
        // this fits if b < 8.75.
        var b1_19 = b.data[1] * 19n;
        var b2_19 = b.data[2] * 19n;
        var b3_19 = b.data[3] * 19n;
        var b4_19 = b.data[4] * 19n;

        // Multiply to get 128-bit coefficients of output
        var c0 = m(a[0], b.data[0]) + m(a[4], b1_19) + m(a[3], b2_19) + m(a[2], b3_19) + m(a[1], b4_19);
        var c1 = m(a[1], b.data[0]) + m(a[0], b.data[1]) + m(a[4], b2_19) + m(a[3], b3_19) + m(a[2], b4_19);
        var c2 = m(a[2], b.data[0]) + m(a[1], b.data[1]) + m(a[0], b.data[2]) + m(a[4], b3_19) + m(a[3], b4_19);
        var c3 = m(a[3], b.data[0]) + m(a[2], b.data[1]) + m(a[1], b.data[2]) + m(a[0], b.data[3]) + m(a[4], b4_19);
        var c4 = m(a[4], b.data[0]) + m(a[3], b.data[1]) + m(a[2], b.data[2]) + m(a[1], b.data[3]) + m(a[0], b.data[4]);
    ...
    Add(x: FieldElement) : FieldElement
    {
        var f = new FieldElement(this.data);
        for (var i = 0; i < 5; i++)
        {
            f.data[i] += x.data[i];
        }
    ...
    {
        var res = new FieldElement(this.data);
        res.data[0] ^= a.data[0];
        res.data[1] ^= a.data[1];
        res.data[2] ^= a.data[2];
        res.data[3] ^= a.data[3];
    ...
    ToBytes(): Uint8Array
    {
        ...
        var limbs = cp.Reduce(this.Clone().data).data;

        var q = (limbs[0] + 19n) >> 51n;
        q = (limbs[1] + q) >> 51n;
        q = (limbs[2] + q) >> 51n;
        q = (limbs[3] + q) >> 51n;
        q = (limbs[4] + q) >> 51n;
    ...
    Invert() : FieldElement
    {
        ...
        var t20 = r[0].Pow2k(5);            // 254..5
        var t21 = t20.Mul(r[1]);              // 254..5,3,1,0
...
export var SQRT_M1 = new FieldElement([1718705420411056n, 234908883556509n, 2233514472574048n, 2117202627021982n, 765476049583133n]);